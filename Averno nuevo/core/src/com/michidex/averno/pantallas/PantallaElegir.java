		...
		if(!Config.empieza){
			...
			Render.limpiarPantalla(0, 0, 0);
				...
				if(opc==1) {
					...
					if(entradas.isEnter()) {
						Render.averno.getCliente().enviarMensaje("Enzo"+"!"+Render.averno.getCliente().getId());
					}
				}else {
					...
					if(entradas.isEnter()) {
						Render.averno.getCliente().enviarMensaje("Marisa"+"!"+Render.averno.getCliente().getId());
					}
				}
